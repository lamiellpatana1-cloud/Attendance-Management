...
public class App {
    ...
    static {
        ...
        try {
            ...
            DB_URL = String.format(
                "jdbc:mysql://%s:%s/%s?useSSL=%s&serverTimezone=%s&allowPublicKeyRetrieval=true&connectTimeout=10000&socketTimeout=30000",
                dbHost,
                dbPort,
                dbName,
                dbSsl,
    ...
    static class HealthHandler implements HttpHandler {
        public void handle(HttpExchange ex) throws IOException {
            ...
            int statusCode = DATABASE_READY ? 200 : 503;
            sendJson(ex, statusCode,
    ...
    static class LoginHandler implements HttpHandler {
        public void handle(HttpExchange ex) throws IOException {
            if (ex.getRequestMethod().equals("OPTIONS")) { sendJson(ex, 200, "{}"); return; }
            if (!DATABASE_READY) {
                sendJson(ex, 503, "{\"error\":\"Database is still starting. Please try again in a few seconds.\"}");
                return;
            }
            try {
                Map<String, String> b = parseJson(readBody(ex));
                ...
                ResultSet rs = query(
                    ...
                    " COALESCE(t.full_name, s.full_name, u.username) AS full_name," +
                    " COALESCE(t.profile_picture, s.profile_picture, '') AS profile_picture," +
                    " s.id AS student_profile_id, t.id AS teacher_profile_id, s.student_id AS student_number" +
                    " FROM users u" +
                    " LEFT JOIN teachers t ON t.user_id = u.id AND COALESCE(t.is_archived,FALSE)=FALSE" +
                    " LEFT JOIN students s ON s.user_id = u.id AND COALESCE(s.is_archived,FALSE)=FALSE" +
                ...
                if (rs.next()) {
                    ...
                    user.put("role",      role);
                    user.put("full_name", rs.getString("full_name"));
                    user.put("profile_picture", rs.getString("profile_picture") == null ? "" : rs.getString("profile_picture"));
                    String profileId = role.equals("student")
                        ? rs.getString("student_profile_id")
                        : role.equals("teacher")
                            ? rs.getString("teacher_profile_id")
                    ...
                    sendJson(ex, 200,
                        ...
                        ",\"name\":\""  + escapeJson(user.get("full_name")) + "\"" +
                        ",\"id\":\""    + escapeJson(user.get("id"))        + "\"" +
                        ",\"profile_id\":\"" + escapeJson(user.get("profile_id")) + "\"" +
                        ",\"student_number\":\"" + escapeJson(user.get("student_number")) + "\"" +
                        ",\"profile_picture\":\"" + escapeJson(user.get("profile_picture")) + "\"}");
                } else {
